...
from it with `inherits SystemTrayIcon` instead of placing it inside a [Window](/master/docs/slint/reference/window/window.md).
...
:::note{Note}
A `SystemTrayIcon` must contain exactly one [Menu](/master/docs/slint/reference/window/contextmenuarea.md#menu) child, and that `Menu` must not
