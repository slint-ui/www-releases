...
In addition, call the [math functions](/master/docs/slint/reference/global-functions/math.md) in postfix style on any numeric value,
...
A brush is a special type that can be either initialized from a `color` or a `gradient`. See [Colors & Brushes](/master/docs/slint/reference/property-types/colors-and-brushes.md).
...
RGB color with an alpha channel, with 8 bit precision for each channel. CSS color names as well as the hexadecimal color encodings are supported, such as #RRGGBBAA or #RGB. See [Colors & Brushes](/master/docs/slint/reference/property-types/colors-and-brushes.md).
...
The `MouseCursor` type is used in the [TouchArea element](/master/docs/slint/reference/gestures/toucharea.md).
...
which is exposed as [Platform.decimal-separator](/master/docs/slint/reference/platform.md#decimal-separator) and defaults to the dot (`.`).
...
See the [translations guide](/master/docs/slint/guide/development/translations.md) for how the decimal separator is determined.