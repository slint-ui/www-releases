...
from it with `inherits SystemTrayIcon` instead of placing it inside a [Window](/master/docs/slint/reference/generated/window/window.md).
...
:::note{Note}
A `SystemTrayIcon` must contain exactly one [Menu](/master/docs/slint/reference/generated/window/context-menu-area.md#menu) child, and that `Menu` must not
...
icon. Its structure is the same as for [MenuBar](/master/docs/slint/reference/generated/window/menu-bar.md) and `ContextMenuArea`: use
`MenuItem` for entries, nested `Menu` elements for sub-menus, and `MenuSeparator` for
separators. See [Menu](/master/docs/slint/reference/generated/window/context-menu-area.md#menu) for the properties and callbacks available on those
elements.
