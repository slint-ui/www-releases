...
Slint's [KeyBinding](/1.17.0/docs/slint/reference/keyboard-input/focusscope.md#keybinding) elements and the built-in [keys](/1.17.0/docs/slint/reference/primitive-types.md#keys) type, which is constructed from the `@keys` macro.
...
Use the [KeyBinding element](/1.17.0/docs/slint/reference/keyboard-input/focusscope.md#keybinding) inside a `FocusScope` to declare key bindings (or keyboard shortcuts).
The `KeyBinding`'s `keys` property takes an instance of the [keys type](/1.17.0/docs/slint/reference/primitive-types.md#keys) and invokes the `activated` callback when the key combination is detected.
