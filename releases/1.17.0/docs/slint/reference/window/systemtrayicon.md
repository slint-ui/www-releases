...
from it with `inherits SystemTrayIcon` instead of placing it inside a [Window](/1.17.0/docs/slint/reference/window/window.md).
...
:::note{Note}
A `SystemTrayIcon` exported alongside a `Window` doesn't share [globals](/1.17.0/docs/slint/guide/language/coding/globals.md) with that window — each instance gets its own copy.
You may need to initialize the relevant globals on each instance, the same way you would across multiple windows.
:::

:::note{Note}
A `SystemTrayIcon` must contain exactly one [Menu](/1.17.0/docs/slint/reference/window/contextmenuarea.md#menu) child, and that `Menu` must not
...
icon. Its structure is the same as for [MenuBar](/1.17.0/docs/slint/reference/window/window.md#menubar) and `ContextMenuArea`: use
`MenuItem` for entries, nested `Menu` elements for sub-menus, and `MenuSeparator` for
separators. See [Menu](/1.17.0/docs/slint/reference/window/contextmenuarea.md#menu) for the properties and callbacks available on those
elements.
