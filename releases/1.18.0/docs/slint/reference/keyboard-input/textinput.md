...
You should probably not use this directly, but instead use the [LineEdit](/1.18.0/docs/slint/reference/std-widgets/views/lineedit.md) or [TextEdit](/1.18.0/docs/slint/reference/std-widgets/views/textedit.md) component.
...
`TextInput` sets this to `true` when it's focused. Only then it receives [KeyEvent](/1.18.0/docs/slint/reference/keyboard-input/overview.md)s.
</SlintProperty>
...
Invoked when a key is pressed, the argument is a [KeyEvent](/1.18.0/docs/slint/reference/keyboard-input/overview.md) struct. Use this callback to
...
Invoked when a key is released, the argument is a [KeyEvent](/1.18.0/docs/slint/reference/keyboard-input/overview.md) struct. Use this callback to
handle keys before `TextInput` does. Return `accept` to indicate that you've handled the event, or return
`reject` to let `TextInput` handle it.
...
### clear-focus()
Call this function to remove keyboard focus from this `TextInput` if it currently has the focus. See also [FocusHandling](/1.18.0/docs/slint/guide/development/focus.md).
