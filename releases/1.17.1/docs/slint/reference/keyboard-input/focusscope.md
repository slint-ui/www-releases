...
The `FocusScope` can react to [keyboard shortcuts](/1.17.1/docs/slint/reference/keyboard-input/overview.md#key-bindings) using the [KeyBinding element](/1.17.1/docs/slint/reference/keyboard-input/focusscope.md#keybinding), and exposes callbacks to handle key events manually.
...
The [KeyEvent](/1.17.1/docs/slint/reference/keyboard-input/overview.md) has a text property, which is a character of the key entered.
When a non-printable key is pressed, the character will be either a control character,
or it will be mapped to a private unicode character. The mapping of these non-printable, special characters is available in the [KeyEvent](/1.17.1/docs/slint/reference/keyboard-input/overview.md) namespace
...
### focus()
Call this function to transfer keyboard focus to this `FocusScope`, to receive future [KeyEvent](/1.17.1/docs/slint/reference/keyboard-input/overview.md)s.

### clear-focus()
Call this function to remove keyboard focus from this `FocusScope` if it currently has the focus. See also [FocusHandling](/1.17.1/docs/slint/guide/development/focus.md).

## Callbacks

### capture-key-pressed(event: KeyEvent) -> EventResult
This function is called during key event handling, *before* `key-pressed` is called. Use this to intercept key press events. The returned [EventResult](/1.17.1/docs/slint/reference/global-structs-enums.md#eventresult)
...
This function is called during key event handling, *before* `key-released` is called. Use this to intercept key release events. The returned [EventResult](/1.17.1/docs/slint/reference/global-structs-enums.md#eventresult)
indicates whether to accept or reject the event. Rejected events are forwarded to the parent element.

### key-pressed(event: KeyEvent) -> EventResult
Invoked when a key is pressed, the argument is a [KeyEvent](/1.17.1/docs/slint/reference/keyboard-input/overview.md) struct. The returned [EventResult](/1.17.1/docs/slint/reference/global-structs-enums.md#eventresult)
indicates whether to accept or reject the event. Rejected events are forwarded to the parent element.

### key-released(event: KeyEvent) -> EventResult
Invoked when a key is released, the argument is a [KeyEvent](/1.17.1/docs/slint/reference/keyboard-input/overview.md) struct. The returned [EventResult](/1.17.1/docs/slint/reference/global-structs-enums.md#eventresult)
indicates whether to accept or reject the event. Rejected events are forwarded to the parent element.

### focus-changed-event(reason: FocusReason)
Invoked when the focus on the `FocusScope` has changed. The argument is a a [FocusReason](/1.17.1/docs/slint/reference/global-structs-enums.md#focusreason) enum containing the reason for focus change.

### focus-gained(reason: FocusReason)
Invoked when the `FocusScope` gains focus. The argument is a a [FocusReason](/1.17.1/docs/slint/reference/global-structs-enums.md#focusreason) enum containing the reason for focus gain.

### focus-lost(reason: FocusReason)
Invoked when the `FocusScope` loses focus. The argument is a a [FocusReason](/1.17.1/docs/slint/reference/global-structs-enums.md#focusreason) enum containing the reason for focus loss.
...
See [Key Bindings](/1.17.1/docs/slint/reference/keyboard-input/overview.md#key-bindings) for details.

### Properties of `KeyBinding`

#### keys
<SlintProperty propName="keys" typeName="keys">
The [keys](/1.17.1/docs/slint/reference/primitive-types.md#keys) to match against incoming key events.
</SlintProperty>
